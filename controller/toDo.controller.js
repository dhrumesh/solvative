...
const allList = async (req, res) => {
  try {
    ...
    const user = await userModel.findById(id);
    if (!user) return res.status(406).json({ message: "user is not valid" });

    if (user.isAdmin == false)
      return res.status(404).json({ message: "you are not able get this data" });

      const all = await toDoModel.find({status : params}).populate('userId', 'name')
